...
func JsonParsing1() {
	...
	JsonParse.Load("F:/GOCode/testdata/SimpleStorage.json", &v)
	//fmt.Println(v)
	fmt.Println(v.IP)
...
//读文件路径测试
func Readfile(){
	b, err := ioutil.ReadFile("F:/GOCode/testdata/SimpleStorage.json")
	if err != nil {
		fmt.Print(err)
	}
	fmt.Println(b)
...
func JsonParsing2() {
	...
	content, err = file_get_contents("F:/GOCode/testdata/SimpleStorage.json")
